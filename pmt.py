...
def replace_in_file(file_path, replacements):
    try:
        ...
        for replacement in replacements:
            old_content = content
            content = content.replace('"' + replacement['replace'] +'"', '"' + replacement['to'] + '"')
            if old_content != content:
                print("注入成功：" + file_path + ': "' + replacement['replace'] +'" 到 ' + '"' + replacement['to'] + '"')
...
def check_config_before_using(config, args):
    ...
    if 'dirname' in config:
        if os.path.basename(args[3]) != config['dirname']:
            print_warning('你填写的路径可能不是正确的路径，该汉化的作者指引你将汉化使用到这个文件夹：' + config['dirname'])
        else:
            print("正确无误")
    if 'early-init' in config or 'lately-init' in config:
        print_warning('该汉化附加了一个 Python 脚本，如果这是恶意脚本，它将足以毁坏您的计算机，请确认您真的相信该汉化包。')
        if 'early-init' in config and not os.path.isfile('./' + os.path.dirname(args[2]) + '/' + config['early-init']):
            print_error("找不到附带的 Python 脚本，该汉化包可能已经损坏：" + config['early-init'])
            return False
        if 'lately-init' in config and not os.path.isfile('./' + os.path.dirname(args[2]) + '/' + config['lately-init']):
            print_error("找不到附带的 Python 脚本，该汉化包可能已经损坏：" + config['lately-init'])
            return False
    print('')
    while True:
                        ...
                        for line_number, line in enumerate(f, 1):
                            if contains_chinese(line):
                                ...
                                for result in results:
                                    localize_list.append((file_path[len(local_path) + 1:], line_number, result[0], result[2]))
                except UnicodeDecodeError:
                    pass
        if dump_mode:
            ...
            for file_path, line_number, col_number, local_str in localize_list:
                print("{\"replace\": " + extract_substring(inter_path + '/' + file_path, line_number, col_number)+", \"to\": " + local_str + "},")
    except Exception as e:
        print_warning("由于未知原因，程序收到了一个错误，以上不是全部的条目。请使用 `"+sys.argv[0]+" --transform-dump "+sys.argv[2]+" "+sys.argv[3]+"` 进行调试。")
...
def unzip(config, basedir):
    ...
    move_folders(basedir + '.pmt-tmp/data', basedir + config['extract-data-to'], config['extract-data'])
    ...
    try:
        if sys.argv[1] == "-h":
            ...
            if 'early-init' in config:
                ...
                os.system("python ./"+os.path.dirname(sys.argv[2]) + '/' + config['early-init'])
            if 'unzip' in config and config['unzip'] == True:
                unzip(config, sys.argv[3] + '/')
            replace_in_folder(sys.argv[3], config['replacements'])
            os.system("python ./"+os.path.dirname(sys.argv[2]) + '/' + config['lately-init'])
            print("汉化注入完成")
        elif sys.argv[1] == "-t":
            ...
            print("本程序属于公有领域且没有任何担保。\n使用本软件的任何后果由您自己承担。")
        else:
            print("未知选项\"" + sys.argv[1] + "\"，请检查你传入的参数，以下是本程序的文档：\n\n")
            print(__doc__)
    except Exception as e:
